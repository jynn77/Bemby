...
router.post('/', (req, res) => {
  const { name, phoneNumber, apiId, apiHash } = req.body as Record<string, string>;
  ...
  ).run(name, phoneNumber, Number(apiId), apiHash);
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(
    'UPDATE tg_accounts SET name = ?, phone_number = ?, api_id = ?, api_hash = ? WHERE id = ?'
  ).run(name ?? existing.name, phoneNumber ?? existing.phone_number, Number(apiId ?? existing.api_id), apiHash ?? existing.api_hash, req.params.id);
...
router.post('/:id/auth/request', async (req, res) => {
  ...
  try {
    await requestCode(account.id, account.api_id, account.api_hash, account.phone_number);
